...
draft: 1
---
<a class="nu-button" href="/spring2020/course-files/projects/project02.zip" target="_blank">
...
pip3 install pip install --upgrade certifi   # for SSL certificates
```
### B. Download the starter files
Click the "Project Starter Files" button above.

### C. Get the API access token
1. Modify the `apis/authentication.py` file (located inside of the apis folder) and add the course API key. To do this: 
   * Navigate to Canvas > Assignments > Project 2
   * Replace lines 8-16 of `apis/authentication.py` with the following line...
   <br>`API_TUTOR_TOKEN = '???'` <br>...but replace the ***???*** with the *actual* token.
1. Run the verification script to make sure you have all of the dependencies installed.

### D. Run the verification script
Navigate to the directory where your project code is stored and type:
```bash
python3 verify_configuration.py
```
If you get a "success" message, then you're ready to begin the project. Otherwise, talk to your TA and they can help you troubleshoot.
...
1. Provide a way for the user to search for and display artists
